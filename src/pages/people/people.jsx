              ...
              {people.map(({ data, title, id }) => (
                <div key={id}>
                  ...
                  <div className='my-0 mx-auto md:mx-0 md:w-auto flex flex-wrap'>
                    {data.map((d, index) => (
                      <div key={index} className='md:w-[29%] mr-5 mb-6 md:mb-36 '>
                        <img
                          src={d.image}
                          alt={d.name}
                          className='object-cover w-full h-[168px] md:h-[311px] w-[132px] md:w-[244px]'
                        />
                        <p className='text-sm md:text-3xl font-rubik mt-12 mb-3'>{d.name}</p>
                        <p className='text-sm md:text-base mb-3 font-light max-w-[140px] md:max-w-full'>{d.title}</p>
